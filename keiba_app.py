...
if st.session_state.result:
    ...
    with st.expander("⚙️ 脚質手動補正（クリックで開く）", expanded=True):
        ...
        # --- Improve UI readability by grouping horses ---
        # 出馬表順（枠順ベース）に並び替え
        if "gate" in df.columns:
    ...
    with top_tabs[4]:
        ...
        with st.expander("オッズ手動入力・補正", expanded=False):
            st.caption("自動取得に失敗したときのみ使用してください")
            manual_win2 = st.text_input("単勝オッズ再入力（カンマ区切り）", key="manual_win2")
            manual_place2 = st.text_input("複勝オッズ再入力（カンマ区切り）", key="manual_place2")
            if st.button("オッズを再適用"):
                try:
                    win_list = [float(x.strip()) for x in manual_win2.split(",")] if manual_win2 else []
                    place_list = [float(x.strip()) for x in manual_place2.split(",")] if manual_place2 else []
                    ...
                    result = refresh_result_payload(result)
                    st.success("オッズを再適用しました。最新のAI評価に更新済みです。")
                except Exception:
                    st.error("オッズ入力形式が不正です")